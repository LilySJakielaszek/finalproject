...
Side by side box plots of the starting median salaries vs the mid-career median salaries for all schools.
```{r}
StartingMedianSalary <- (SchoolSalaries$Starting_Median_Salary)
MidCareerMedianSalary <- (SchoolSalaries$Mid_Career_Median_Salary)
Medians <- c("Starting", "MidCareer")

boxplot(StartingMedianSalary, MidCareerMedianSalary, names = Medians, ylab = "Median Salary", main = "Starting Median Salary vs MidCareer Median Salary for all Schools")
```
...
SchoolSalaries %>% # This code will find the mean for mid career salary
  summarise(mean = sum(Mid_Career_Median_Salary)/349)
MidCareerAvgMedianSalary <-
  SchoolSalaries %>%
  mutate(Above_Avg_MidCareer_Median_Salary = ifelse(Mid_Career_Median_Salary <= 83609.74, "yes", "no")) # This code uses the mean to indicate an above or below for each school
MidCareerAvgMedianSalary
```
...
```{r}
MidCareerMedianSalariesPlot <- party::ctree(Above_Avg_MidCareer_Median_Salary ~ Region + School_Type, data = MidCareerAvgMedianSalary)
plot(MidCareerMedianSalariesPlot, type = "simple")
```
...
CompleteCases <- 
  StartingAvgMedianSalary %>%
  select(Above_Avg_Starting_Median_Salary, Starting_Median_Salary,Mid_Career_Median_Salary,School_Type, Region) %>%
  na.omit() 
CompleteCases<-CompleteCases%>%mutate(School_Type=as.factor(School_Type), Region=as.factor(Region))
mod1 <- party::ctree(Above_Avg_Starting_Median_Salary=="yes"~ School_Type+Region, data = CompleteCases)
...
In conclusion, a student should choose the college California Institute of Technology (CIT) , the major Physician Assistant , and the occupation Chief Executive. We chose these factors by the highest starting median salary. 
...
whoIsPregnant2 <- 
  NCHS %>%
  filter(sex == "female") %>%
  ctree(pregnant == "yes" ~ age + income + ethnicity + height + smoker + bmi, 
        data = .)
whoIsPregnant2
```
