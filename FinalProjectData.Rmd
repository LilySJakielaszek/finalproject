...
Now we will make a graph to demostrate the importance of school type and region in determining whether a school will have an above average mid career median salary

```{r}
MidCareerAvgMedianSalary %>%
  head(20) %>%
  ggplot() +
  geom_point(aes(x = School_Name, y = Mid_Career_Median_Salary, color = Region)) +
  facet_wrap(~ Above_Avg_MidCareer_Median_Salary) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
